...
def get_players(db: Session, skip: int = 0, limit: int = 100,
    ...
    if min_last_changed_date:
        query = query.filter(
            models.Player.last_changed_date >= min_last_chaned_date
        )
    if first_name:
        query = query.filter(models.Player.last_name == first_name)
    if last_name:
...
def get_leagues(db: Session, league_id: int = None):
